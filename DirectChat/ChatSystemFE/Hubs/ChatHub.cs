...
namespace ChatWebSocketAPI.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        public async Task AssignSession(string sessionId, string adminId)
        {
            if (!Guid.TryParse(sessionId, out var sessionGuid) || !Guid.TryParse(adminId, out var adminGuid))
            {
                await Clients.Caller.SendAsync("Error", "Invalid session or admin ID");
                return;
            }
            ...
            try
            {
                var updatedSession = await _chatService.AssignSessionAsync(sessionGuid, adminGuid);
                if (updatedSession != null)
                {
                    ...
                    // Notify the customer
                    await Clients.Group($"Session_{sessionId}").SendAsync("SessionAssigned", updatedSession);
                }
            }
            catch (Exception ex)
            {
